...
export class DatasetTree implements vscode.TreeDataProvider<ZoweNode> {
    ...
    private persistenceSchema: string = "Zowe-Persistent-Favorites";
    ...
    public async onDidChangeConfiguration(e) {
        if (e.affectsConfiguration(this.persistenceSchema)) {
            ...
            if (!setting.persistence) {
                setting.favorites = [];
                setting.history = [];
                await vscode.workspace.getConfiguration().update(this.persistenceSchema, setting, vscode.ConfigurationTarget.Global); // MISSED
            }
        }
    }
    ...
    public async retrieveHistory(node: ZoweNode, log: Logger) {
        log.debug(localize("enterPattern.log.debug.prompt", "Prompting the user for a data set pattern"));
        let pattern: string = this.defaultDialogText;
        if (node.contextValue === "session") {
            ...
            if (pattern === this.defaultDialogText) {
            ...
            await this.addSession(log, session);
