...
export class ZoweUSSNode extends ZoweTreeNode implements IZoweUSSTreeNode {
    ...
    public async deleteUSSNode(ussFileProvider: Types.IZoweUSSTreeType, _filePath: string, cancelled: boolean = false): Promise<void> {
        ...
        // Close any open editors for the deleted file or folder
        const localPath = this.resourceUri?.fsPath;
        if (localPath) {
            if (this.isFolder) {
                // For folders, close all open editors with paths under this folder
                ...
                // For files, close the specific file if open
